...
const MapWithSearchBox = (props) => {
  ...
  const { updateValue, name, searchAddress } = props;
  const [locationInput, setLocationInput] = useState({
    searchedLocation: searchAddress,
  });
  ...
  useEffect(() => {
    setLocationInput({ searchedLocation: searchAddress });
    // if (searchBox) {
    //   let event = new KeyboardEvent('keypress', { key: 'a' });
    // document
    //   .getElementById('propertyInputSearch')
    //   .dispatchEvent(new Event('focus'));
    // document.getElementById('propertyInputSearch').dispatchEvent(event);
    // }
  }, [searchAddress]);
  ...
  const onPlacesChanged = () => {
    ...
    if (places.length > 0) updateValue(places);
  };
  ...
  const onDragEndFunc = (marker) => {
    ...
    if (dragNDropData.length > 0) updateValue(dragNDropData);
  };
